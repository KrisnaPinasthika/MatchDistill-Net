...
def train(model, args, epochs=10, experiment_name="test-01x", lr=0.000359, root=".", device=None, optimizer_state_dict=None):
    ...
    # if args.rank == 0:  # To prevent multiple unnecessary initializations
    ...
    for epoch in range(args.epoch, epochs):
        print(f"Epoch : {epoch}")
        if should_log: 
            wandb.log({"Train/Epoch": epoch + 1}, step=step)
        ...
        #! Todo: Student - Main Loss  
            ...
            # Entropy Calculation --------------------------------------------------
            l_reg = 0
            for idx, attn_w in enumerate(attn_weights):
                p = attn_w.clamp(min=1e-8)              #?  Make sure p > 0
                
            #? Flatten the list of lists of attention weights
            all_individual_attns = [tensor for sublist in attn_weights for tensor in sublist]
            for attn_tensor in all_individual_attns:
                p = attn_tensor.clamp(min=1e-8)         #?  Make sure p > 0
                entropy = -torch.sum(p * torch.log(p), dim=-1)  
                l_reg += entropy.mean()

            l_reg /= len(attn_weights)
            if all_individual_attns:
                l_reg /= len(all_individual_attns)
            #--------------------------------------------------
            ...
            if (step) % 5 == 0:
                wandb.log({f"Train/{criterion_softdelta.name}": round(l_soft_delta.item(), 5)}, step=step)
            ...
            if should_write and step % args.validate_every == 0:
                ...
                ################################# Validation loop ##################################################
                model.eval()
                metrics, val_si = validate(args, model, test_loader, [criterion_softdelta, criterion_bins], epoch, epochs, device)
                
                wandb.log({f"Test/TripleLoss": round(val_si.get_value(), 5) }, step=step)
                ...
                # model_io.save_checkpoint(midas, optimizer, epoch, f"Midas_{experiment_name}_{run_id}_latest.pt",
                #                          root=os.path.join(root, "checkpoints"))
...
def validate(args, model, test_loader, criterions, epoch, epochs, device='cpu'):
    with torch.no_grad():
        ...
        for batch in tqdm(test_loader, desc=f"Epoch: {epoch + 1}/{epochs}. Loop: Validation") if is_rank_zero(
            ...
            should_interpolate = True
            l_softdelta = criterions[0](pred, depth, mask=mask, interpolate=should_interpolate)
            bin_edges = None
            if bin_edges != None:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n-bins', '--n_bins', default=128, type=int, help='number of bins/buckets to divide depth range into')
    ...
    parser.add_argument('--bs', default=21, type=int, help='batch size')
