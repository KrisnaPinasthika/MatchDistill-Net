...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, backbone_name):
        ...
        # backbone_name: eff_b5, mobilenetv3_small, mobilenetv3_large 
        self.backbone_name = backbone_name 
        
        if self.backbone_name == 'eff_b5':
            self.cnn = efficientnet_b5(weights=EfficientNet_B5_Weights.IMAGENET1K_V1).features
            self.cnn_channels = [24, 40, 64, 176, 2048]
            self.take_idx = [2, 3, 4, 6, 9]
            
        elif self.backbone_name == 'mobilenetv3_small':
            self.cnn = mobilenet_v3_small(weights=MobileNet_V3_Small_Weights.IMAGENET1K_V1).features
            self.cnn_channels = [16, 16, 24, 48, 96, 576]
            self.take_idx = [1, 2, 4, 9, 13]
            
        elif self.backbone_name == 'mobilenetv3_large':
            self.cnn = mobilenet_v3_large(weights=MobileNet_V3_Large_Weights.IMAGENET1K_V1).features
            self.cnn_channels = [16, 24, 40, 112, 960]
            self.take_idx = [2, 4, 7, 13, 17]
        else:
            print('Check your backbone again ^.^')
            return None
        
        self.transformer = swin_t(weights=Swin_T_Weights.IMAGENET1K_V1).features[:6]
        self.tf_channels = [96, 192, 384]
        self.act = nn.GELU()
        
        self.tf_conv1 = nn.Conv2d(in_channels=self.tf_channels[0], out_channels=self.cnn_channels[1], kernel_size=1, stride=1, padding='same', bias=False)
        self.tf_conv2 = nn.Conv2d(in_channels=self.tf_channels[1], out_channels=self.cnn_channels[2], kernel_size=1, stride=1, padding='same', bias=False)
        self.tf_conv3 = nn.Conv2d(in_channels=self.tf_channels[2], out_channels=self.cnn_channels[3], kernel_size=1, stride=1, padding='same', bias=False)
        
        self.conv_stabilizer1 = nn.Conv2d(in_channels=self.cnn_channels[1], out_channels=self.cnn_channels[1], kernel_size=1, stride=1, padding='same', bias=False)
        self.conv_stabilizer2 = nn.Conv2d(in_channels=self.cnn_channels[2], out_channels=self.cnn_channels[2], kernel_size=1, stride=1, padding='same', bias=False)
        self.conv_stabilizer3 = nn.Conv2d(in_channels=self.cnn_channels[3], out_channels=self.cnn_channels[3], kernel_size=1, stride=1, padding='same', bias=False)
        
        self.conv_matcher1 = ConvMatcher(self.cnn_channels[1]*2, self.cnn_channels[1], dilations=(3, 5))
        self.conv_matcher2 = ConvMatcher(self.cnn_channels[2]*2, self.cnn_channels[2], dilations=(3, 5, 7))
        self.conv_matcher3 = ConvMatcher(self.cnn_channels[3]*2, self.cnn_channels[3], dilations=(7, 12, 24))
        
    
    def getStudentRelatedLayer(self):
        return [self.cnn]
    
    def getTeacherRelatedLayer(self):
        return [
                self.transformer, self.tf_conv1, self.tf_conv2, self.tf_conv3, 
                self.conv_stabilizer1, self.conv_stabilizer2, self.conv_stabilizer3,
                self.conv_matcher1, self.conv_matcher2, self.conv_matcher3
            ]
    
    def forward(self, x, is_train):
        features_cnn = [x]
        for layer_cnn in self.cnn: 
            pred = layer_cnn(features_cnn[-1])
            features_cnn.append(pred)
            
        eff1 = features_cnn[self.take_idx[0]]
        eff2 = features_cnn[self.take_idx[1]]
        eff3 = features_cnn[self.take_idx[2]]
        eff4 = features_cnn[self.take_idx[3]]
        eff5 = features_cnn[self.take_idx[4]]
        ...
        if is_train:
            ...
            for layer_tf in self.transformer:
                pred = layer_tf(features_transformer[-1])
            ...
            teacher_layers = self.getTeacherRelatedLayer()
            for m in teacher_layers:
                for param in m.parameters():
                    param.requires_grad = False
                    
            features = [eff1, eff2, eff3, eff4, eff5]
...
class NormalDecoderBlock(nn.Module):
    """Some Information about NormalDecoderBlock"""
    def __init__(self, skip_channels, x_channels, out_channels):
        super(NormalDecoderBlock, self).__init__()
        self.conv_sequence = nn.Sequential(
                nn.Conv2d(in_channels=skip_channels+x_channels, out_channels=out_channels, stride=1, kernel_size=3, padding='same'),
                nn.GroupNorm(num_groups=8, num_channels=out_channels),
                nn.GELU(),
                
                nn.Conv2d(in_channels=out_channels, out_channels=out_channels, stride=1, kernel_size=3, padding='same'),
                nn.GroupNorm(num_groups=8, num_channels=out_channels),
                nn.GELU(),
            )
...
class MatchDistillNet(nn.Module):
    ...
    def __init__(self, device, backbone_name, n_bins:int, window_sizes:int, layers:int, 
                    qkv_bias:bool=True, drop_prob:float=0.15, min_val:float=0.001, max_val:float=10.0, 
                    normal_decoder:bool=False, normal_head:bool=False
                    ):
        super(MatchDistillNet, self).__init__()
        self.min_val = min_val  
        self.max_val = max_val  
        self.backbone_name = backbone_name.lower()
        self.encoder = EncoderBlock(self.backbone_name)
        features = self.encoder.cnn_channels
        dec_heads = 4
        
        self.normal_decoder = normal_decoder
        self.normal_head = normal_head
        if self.normal_decoder:
            self.decoder = nn.ModuleList([
                NormalDecoderBlock(x_channels=features[-1], skip_channels=features[-2], out_channels=features[-1]//4),
                NormalDecoderBlock(x_channels=features[-1]//4, skip_channels=features[-3], out_channels=features[-1]//8),
                NormalDecoderBlock(x_channels=features[-1]//8, skip_channels=features[-4], out_channels=features[-1]//16),
                NormalDecoderBlock(x_channels=features[-1]//16, skip_channels=features[-5], out_channels=features[-1]//32),
            ]).to(device)
        else:
            self.decoder = nn.ModuleList([
                DecoderBLock(
        ...
        self.normal_head = normal_head
        if self.normal_head:
            self.normal_head = nn.Conv2d(features[-1]//32, 1, kernel_size=1, stride=1, padding='same')
        else:
            self.head = Head(features[-1]//32, n_bins, min_val, max_val, drop_prob, device).to(device)
        self.act = nn.GELU()
        self.teacher_set2false = False
        
    def forward(self, x, is_train=False):
        if is_train == False and self.teacher_set2false == False:
            teacher_layers = self.encoder.getTeacherRelatedLayer()
            for m in teacher_layers:
                for param in m.parameters():
                    param.requires_grad = False
            self.teacher_set2false = True
        ...
        if self.normal_decoder:
            u1 = self.decoder[0](block4_out, block5)
            u2 = self.decoder[1](block3_out, u1)
            u3 = self.decoder[2](block2_out, u2)
    ...
    def getStudentRelatedLayer(self):
        student_layers = self.encoder.getStudentRelatedLayer()
        for m in student_layers:
...
if __name__ == '__main__': 
    ...
    #     # Buat wrapper agar fvcore tidak mengubah boolean menjadi Tensor
    #     class ModelWrapper(nn.Module):
    #         def __init__(self, base_model, train_flag):
    #             super().__init__()
    #             self.base_model = base_model
    #             self.train_flag = train_flag
                
    #         def forward(self, x):
    #             return self.base_model(x, is_train=self.train_flag)

    #     # Gunakan model yang sudah dibungkus
    #     wrapped_model = ModelWrapper(model, is_train)

    #     # Hitung FLOPs HANYA dengan memberikan argumen Tensor (img)
